...
export function createAccountUI({root,incomingLink=null,onShowGate=()=>{},onAuthenticated=()=>{},onSignedOut=()=>{},onUnavailable=()=>{},onProfile=()=>{},onPreview=()=>{},client=createAccountClient()}={}) {
  let config=null,turnstile=null,link=incomingLink,proof=null,busy=false,disposed=false,revision=0,allowPreview=false;
  let activeView='login',profileTarget=null,currentTerms=null,avatarUrl=null,avatarOwner=null;
  ...
  function show(view='login',message='',tone='info') {
    ...
    if(view==='login') {
      paint(`${form('login',email()+password('Password','password',true),'Sign in')}<div class="account-links">${action('forgot','Forgot password?')}${action('resend','Resend verification')}</div><p class="account-small">New here? Open your personal invitation email to create your account.</p>${allowPreview?`<div class="account-preview-option">${action('preview','Open local synthetic preview')}</div>`:''}`,{title:'Welcome back.',message,tone});return;
    }
    if(view==='signup') {
      if(link?.kind!=='invite'||!link.token){show('invalid');return;}
      paint(`<p class="gate-intro">Use the email address your invitation was sent to. Your role comes from the invitation.</p>${form('signup',email('Invited email')+field('Display name','display_name',{autocomplete:'nickname'})+password()+password('Confirm password','confirm_password'),'Create invited account')}${footerLinks()}`,{title:'Your invitation.',message,tone});return;
    }
    if(view==='forgot'||view==='resend') {
      paint(`<p class="gate-intro">${view==='forgot'?'Request a one-use password reset link.':'Request a new verification link for your account.'}</p>${form(view,email(),view==='forgot'?'Request reset link':'Request verification link')}${footerLinks()}`,{title:view==='forgot'?'Reset your password.':'Verify your email.',message,tone});return;
    }
    if(['verify','reset','email-change'].includes(view)) {
      ...
      paint(`<p class="gate-intro">${change?'Enter the current account email, before this change. The link confirms the new address requested by that account.':reset?'Enter your account email and choose a new password. Completed reset revokes your existing sessions.':'Enter the account email this link was issued for. Verification is completed only after the server accepts the link.'}</p>${form(view,email(change?'Current account email (before the change)':'Account email')+(reset?password('New password')+password('Confirm new password','confirm_password'):''),reset?'Reset password':change?'Confirm email change':'Verify email')}${footerLinks()}`,{title:reset?'Choose a new password.':change?'Confirm your new email.':'Complete verification.',message,tone});return;
    }
    if(view==='verification-pending') {
      paint(`<p class="gate-intro">Check your inbox for a verification link. Provider acceptance is separate from delivery and verification.</p>${footerLinks()}`,{title:'Verification requested.',message,tone});return;
    }
  ...
  async function submit(event) {
    ...
    try {
      ...
      if(kind==='signup'){if(result.verification_required!==true)throw new AccountError('invalid_response','Account verification was not confirmed.');link=null;show('verification-pending','The account service requires email verification.');return;}
      ...
      if(kind==='verify'){if(result.verified!==true)throw new AccountError('invalid_response','Verification was not confirmed.');link=null;show('login','Your email is verified. Sign in to continue.','success');return;}
      if(['reset','password','email-change'].includes(kind)) {
        ...
        link=null;proof=null;avatarUrl=null;avatarOwner=null;onSignedOut();show('login',kind==='email-change'?'Your email was changed and existing sessions were revoked. Sign in with the new address.':'Your password was changed and existing sessions were revoked. Sign in with your new password.','success');return;
      }
      ...
      if(kind==='email'){if(result.verification_required!==true)throw new AccountError('invalid_response','The email change request was not confirmed.');show('email','Verification was requested for the new address. Open its link and enter your original account email to complete the change.','success');return;}
      ...
      if(['logout','logout-all'].includes(kind)){if(result.signed_out!==true)throw new AccountError('invalid_response','Sign-out was not confirmed.');avatarUrl=null;avatarOwner=null;onSignedOut();show('login',kind==='logout-all'?'Your app sessions were revoked on every device.':'You signed out of this app session.','success');}
    } catch(error) {
      if(attempt!==revision||disposed)return;
  ...
  return Object.freeze({client,getUser:user,getConfig:()=>config,mountProfile,show,
    async start({preview=false}={}) {
      ...
      try {config=await client.config();turnstile=createTurnstile(config.turnstile_site_key);}
      ...
      if(link){show(link.kind==='invite'?'signup':link.kind);return true;}
    ...
    handleLink(next) {link=next;proof=null;if(config)show(link?.kind==='invite'?'signup':link?.kind||'login');else onUnavailable(new AccountError('configuration_required','Account access is not available yet.',503),Boolean(link));},
    ...
    dispose(){disposed=true;revision++;link=null;proof=null;avatarUrl=null;avatarOwner=null;client.clear();turnstile?.cancel();document.removeEventListener('submit',submit);document.removeEventListener('click',click);}
  });
}
