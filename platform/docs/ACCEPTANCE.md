...
Current evidence: API12 tests and independent G3 sign-off. Account G2 and M3/M4 peer reviews are recorded in communication-log.md. Worker130 full tests plus the new signed-handler authentication-expiry regression pass. App82 tests,39 syntax checks and all13 imported Git-blob hashes pass; static build and both workflow YAML files validate. Actual local workerd/R2 delivery passed18 checks, including exact installer/manifest bytes, HEAD and denied requests. The earlier workerd event probe remains applicable. Desktop signature positives use injected attestations; an actual unsigned Windows fixture is rejected. Unsigned package inspection is separate from installed/signing evidence. CI and bundle size for the M5 head are recorded at delivery. No platform/account/board deployment occurred. Browser visual checks remain unverified because Chrome blocked the local preview. See DESKTOP-RELEASES.md and each package's scripts for reproducible checks. No milestone is complete until its applicable rows pass or the user explicitly changes scope.
