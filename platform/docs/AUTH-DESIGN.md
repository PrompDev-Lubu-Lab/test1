...
5. Desktop choice was revised with Fable in round08: a hardened Electron BrowserWindow loads the approved deployed HTTPS app origin. Normal human Access login, Turnstile, host-only cookies, Origin and CSRF then follow the browser flow. No cloudflared token adapter, shared service credential or custom PKCE endpoint is shipped. A persistent Chromium partition holds normal browser sessions; the main process alone may bridge its current cookies to a same-origin protected updater under strict request/path restrictions. Actual Electron identity/Turnstile and cookie-bridge validation remain M5 gates. See the desktop design for pinned updater signing limitations.
...
## G2 re-check and M3/M4 review

Fable round07 accepted the G2 fixes at d452828 with no staging code blockers, board aacd0ab. Round08 reviewed d1d7579: M3 had no findings; board preamble and large-edit fixes were required. Those are addressed with dedicated tests in the next delivery. No real identity, account or board activation follows from code review alone.
