...
2. Request order on password routes: validate exact origin, JSON content type and bounded body; reserve per-IP and per-account slots in the auth Durable Object before external calls; verify Turnstile; look up account/token state; perform at most one KDF. Every reservation counts; successful login clears only the account window, never the IP/global ceiling. A global maximum of 60 expensive-operation reservations per minute applies across the Worker. Recovery uses a separate account bucket, still bounded by IP/global limits, so a locked login cannot permanently prevent password reset. The tenth failed login locks that account for ten minutes.
