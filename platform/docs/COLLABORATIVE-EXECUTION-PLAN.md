...
Electron keeps context isolation and sandboxing, disables renderer Node integration, restricts navigation/external links/IPC, and stores session tokens through supported OS credential storage only. Reject insecure plaintext fallbacks. Desktop login needs the real external-browser Access + app session exchange with one-use state/PKCE; do not embed a Cloudflare service token to bypass humans. Private update downloads must support a documented per-session request transport without secrets in manifest URLs.
