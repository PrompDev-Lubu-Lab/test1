...
`desktop/settings.example.json` is a closed review example. Private settings select exact app and Access origins, a small external-link allowlist, channel, update enablement, publisher names and certificate thumbprints. `publisherNames` contains the exact certificate simple/common name returned by `X509Certificate2.GetNameInfo(SimpleName, false)` (for example `Example Publisher`), not the full `CN=..., O=..., C=...` subject. `certificateThumbprints` contains uppercase 40-character SHA-1 certificate thumbprints; it is a certificate identity pin, separate from the installer SHA-512. Credentials never belong in this file. The packaging script validates it and embeds `settings.generated.json` inside the integrity-protected ASAR. Do not move this policy to a mutable external resource. Generated/private settings are ignored by Git.
...
Local evidence: `desktop-review-results.json` records the built unsigned111566442-byte NSIS package and actual fuse/archive checks. `downloads-runtime-results.json` records18 real local workerd/R2 checks. The bounded installer body uses Cloudflare FixedLengthStream so both GET and HEAD retain the exact size; the runtime probe requires the Content-Length header and verifies every byte. These are not production or installed-upgrade results.

The fixed-length response follows the [Cloudflare response contract](https://developers.cloudflare.com/workers/runtime-apis/response/#set-the-content-length-header). The signature process has a bounded 30-second execution budget; the final account receipt still has its independent twenty-second freshness limit. The exact updater dependency is covered by a contract test for its native config/cache/file members. Both packaging jobs pin GitHub Actions to reviewed commit SHAs.
