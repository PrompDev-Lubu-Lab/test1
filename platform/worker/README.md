...
Both modes execute the guarded insert once through Wrangler `--file`, then run a separate read-only `--command` query for that attempt's exact digest against the same database and configuration. Remote file imports return execution totals rather than selected rows, so those totals never confirm an invitation. Only the separate query's exact email, handle, role, expiry and digest match permits link display or the one email attempt. An import or confirmation failure sends nothing, keeps any inserted invitation guarded, removes the temporary digest-only SQL file and requires deliberate operator inspection before another attempt; there is no automatic retry or resend.
