...
Use `--local` only for an explicitly configured local database. The command refuses redirected/CI output, never seeds a password or Access subject, and refuses a second active invitation or any existing account. Open the one-use link privately and complete normal signup and verification. A timeout or uncertain result requires inspecting the database before retrying; a stored but unconfirmed invitation remains closed until expiry or a deliberate operator recovery. The script's real SQLite tests exercise its SQL and exact-row confirmation; local tests do not establish real invitation delivery or signup.

Both modes execute the short guarded insert once through Wrangler `--command`, then run a separate read-only `--command` query for that attempt's exact digest against the same database and configuration. File imports can emit progress text before JSON even with `--json`, so this script never uses `--file` or creates a temporary SQL file. The child log level is pinned to `log` because Wrangler prints D1 JSON through that level; unexpected non-JSON output is still rejected without attempting to extract a JSON substring. Both child processes exclude the email API token, and the SQL contains the invite digest rather than the raw invitation token. Only the separate query's exact email, handle, role, expiry and digest match permits link display or the one email attempt. An insert or confirmation failure sends nothing, keeps any inserted invitation guarded and requires deliberate operator inspection before another attempt; there is no automatic retry or resend.

The exported `executeBootstrapSql(settings, insertSql, confirmationSql, mode)` can also be checked independently with `SELECT 1` for both SQL arguments. That read-only executor check neither prepares an invitation nor enters the email flow; it verifies the installed Wrangler path, child environment and strict JSON transport before a separately approved bootstrap attempt.
